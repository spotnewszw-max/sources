...
#!/usr/bin/env python3
"""
ZimDaily scraper with enhanced features
"""
...
def scrape_zimdaily(max_articles=10):
    """
    Scrape ZimDaily news with image support
    """
    articles = []
    
    # Multiple URLs to try
    urls_to_try = [
        "https://www.zimdaily.com/",
        "https://zimdaily.com/",
        "https://www.zimdaily.com/category/news/",
        "https://zimdaily.com/category/news/"
    ]
    
    # Use RobustSession (prefers cloudscraper if available)
    session_helper = RobustSession("ZimDaily")
    ...
    for url in urls_to_try:
        try:
            logger.info(f"Trying ZimDaily URL: {url}")
            ...
            if response.status_code == 200:
                ...
                successful_url = url
                logger.info(f"Successfully accessed ZimDaily: {url}")
                break
            else:
    ...
    if not soup:
        logger.error("Could not access any ZimDaily URLs")
        return articles
    ...
    try:
        ...
        if not article_containers:
            ...
            for link in all_links:
                ...
                if (text and len(text) > 20 and len(text) < 200 and
                    ('zimdaily.com' in href or href.startswith('/')) and
        ...
        for i, container in enumerate(article_containers):
            ...
            try:
                ...
                logger.info(f"Processing ZimDaily article: {title[:50]}...")
                
                # Get article content and image
                content, image_url = get_zimdaily_article_content(link, scraper)
                ...
                logger.warning(f"Error processing ZimDaily article {i}: {e}")
                continue
        
        logger.info(f"Successfully scraped {len(articles)} articles from ZimDaily")
        return articles
        
    except Exception as e:
        logger.error(f"Error scraping ZimDaily: {e}")
        return articles

def get_zimdaily_article_content(url, scraper):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if content_parts:
                ...
                source_attribution = f"<p><em>Source: <a href='{url}' target='_blank'>ZimDaily</a></em></p>"
...
def scrape_webpage(url: str = None, max_articles: int = 10) -> List[Dict]:
    ...
    articles = scrape_zimdaily(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    articles = scrape_zimdaily(max_articles=3)
    print(f"Found {len(articles)} articles from ZimDaily")
