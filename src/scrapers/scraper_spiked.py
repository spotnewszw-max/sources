...
def scrape_spiked(max_articles: int = 12) -> List[Dict]:
    articles: List[Dict] = []

    start_urls = [
        "https://spikedmedia.co.zw/",
        "https://www.spikedmedia.co.zw/",
        "https://spikedmedia.co.zw/category/news/",
        "http://spikedmedia.co.zw/",
    ]
    ...
    if not soup or not base_url:
        logger.error("Spiked Media: no homepage accessible")
        return articles
    ...
    if not link_elems:
        ...
        for a in anchors:
            ...
            if any(x in h for x in ["/category/", "/tag/", "/author/", "/page/"]):
                continue
            if (host in h or "spikedmedia.co.zw" in h) and ("/20" in h or "/news/" in h):
                link_elems.append(a)
    ...
    for a in link_elems:
        ...
        try:
            ...
            content_html = body_html + "\n" + build_source_attribution(href, "Spiked Media")
            ...
            logger.warning(f"Spiked Media item failed: {e}")
            continue
...
def scrape_webpage(url: str = None, max_articles: int = 10):
    ...
    articles = scrape_spikedmedia(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    items = scrape_spikedmedia(8)
    print(f"Found {len(items)} Spiked Media articles")
