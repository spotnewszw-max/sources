...
def scrape_businessdailyafrica(max_articles=10):
    """Scrape Businessdailyafrica"""
    articles = []
    ...
    try:
        ...
        if response is None:
            logger.error("Could not access businessdailyafrica.com")
            return articles
        
        if response.status_code != 200:
            logger.error(f"businessdailyafrica.com returned status {response.status_code}")
            return articles
        ...
        logger.error(f"Error scraping Businessdailyafrica: {e}")
        return articles

if __name__ == "__main__":
    articles = scrape_businessdailyafrica(max_articles=5)
