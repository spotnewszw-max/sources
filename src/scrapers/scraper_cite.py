...
#!/usr/bin/env python3
"""
CITE (Centre for Investigative Journalism) scraper with enhanced features
"""
...
def scrape_cite(max_articles=10):
    """
    Scrape CITE news with image support
    """
    articles = []
    ...
    urls_to_try = [
        "https://cite.org.zw/",
        "https://www.cite.org.zw/",
        "https://cite.org.zw/category/news/",
        "https://www.cite.org.zw/category/news/"
    ]
    
    # Use RobustSession (prefers cloudscraper if available)
    session_helper = RobustSession("CITE")
    ...
    for url in urls_to_try:
        try:
            logger.info(f"Trying CITE URL: {url}")
            ...
            if response.status_code == 200:
                ...
                successful_url = url
                logger.info(f"Successfully accessed CITE: {url}")
                break
            else:
    ...
    if not soup:
        logger.error("Could not access any CITE URLs")
        return articles
    ...
    try:
        ...
        if not article_containers:
            ...
            for link in all_links:
                ...
                if (text and len(text) > 20 and len(text) < 200 and
                    ('cite.org.zw' in href or href.startswith('/')) and
        ...
        for i, container in enumerate(article_containers):
            ...
            try:
                ...
                logger.info(f"Processing CITE article: {title[:50]}...")
                
                # Get article content and image
                content, image_url = get_cite_article_content(link, scraper)
                ...
                logger.warning(f"Error processing CITE article {i}: {e}")
                continue
        
        logger.info(f"Successfully scraped {len(articles)} articles from CITE")
        return articles
        
    except Exception as e:
        logger.error(f"Error scraping CITE: {e}")
        return articles

def get_cite_article_content(url, scraper):
            ...
            if getattr(response, "status_code", 0) == 200 and getattr(response, "content", b""):
                ...
                if not content_parts:
                    ...
                    if parts:
                        ...
                        return html + "\n" + build_source_attribution(url, "CITE"), image_url
...
def scrape_webpage(url: str = None, max_articles: int = 10):
    ...
    articles = scrape_cite(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    articles = scrape_cite(max_articles=3)
    print(f"Found {len(articles)} articles from CITE")
