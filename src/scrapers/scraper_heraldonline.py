...
def scrape_heraldonline(max_articles=10):
    """Scrape Heraldonline"""
    articles = []
    ...
    try:
        ...
        if response is None:
            logger.error("Could not access heraldonline")
            return articles
        
        if response.status_code != 200:
            logger.error(f"Heraldonline returned status {response.status_code}")
            return articles
        ...
        logger.error(f"Error scraping Heraldonline: {e}")
        return articles

if __name__ == "__main__":
    articles = scrape_heraldonline(max_articles=5)
