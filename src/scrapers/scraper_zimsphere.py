...
#!/usr/bin/env python3
"""
ZimSphere scraper with enhanced features
"""
...
def scrape_zimsphere(max_articles=10):
    """
    Scrape ZimSphere news with image support
    """
    articles = []
    
    # Multiple URLs to try
    urls_to_try = [
        "https://www.zimsphere.co.zw/",
        "https://zimsphere.co.zw/",
        "https://www.zimsphere.co.zw/search/label/News",
        "https://zimsphere.co.zw/search/label/News"
    ]
    
    # Use RobustSession (prefers cloudscraper if available)
    from utils.scraper_enhanced import RobustSession
    session_helper = RobustSession("ZimSphere")
    ...
    for url in urls_to_try:
        try:
            logger.info(f"Trying ZimSphere URL: {url}")
            ...
            if response.status_code == 200:
                ...
                successful_url = url
                logger.info(f"Successfully accessed ZimSphere: {url}")
                break
            else:
    ...
    if not soup:
        logger.error("Could not access any ZimSphere URLs")
        return articles
        ...
        if not article_containers:
            ...
            for link in all_links:
                ...
                if (text and len(text) > 20 and len(text) < 200 and
                    ('zimsphere.co.zw' in href or href.startswith('/')) and
                    not any(skip in href.lower() for skip in ['/search/', '/label/', '/author/', '/contact', '/about'])):
        ...
        for i, container in enumerate(article_containers):
            ...
            try:
                ...
                if not title or len(title) < 10:
                    for tag in ['h1', 'h2', 'h3', 'h4', '.title', '.headline', '.entry-title', '.post-title']:
                ...
                logger.info(f"Processing ZimSphere article: {title[:50]}...")
                
                # Get article content and image
                content, image_url = get_zimsphere_article_content(link, scraper)
                ...
                logger.warning(f"Error processing ZimSphere article {i}: {e}")
                continue
        
        logger.info(f"Successfully scraped {len(articles)} articles from ZimSphere")
        return articles
        
    except Exception as e:
        logger.error(f"Error scraping ZimSphere: {e}")
        return articles

def get_zimsphere_article_content(url, scraper):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if content_parts:
                ...
                source_attribution = f"<p><em>Source: <a href='{url}' target='_blank'>ZimSphere</a></em></p>"
...
def scrape_webpage(url: str = None, max_articles: int = 10):
    ...
    articles = scrape_zimsphere(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    articles = scrape_zimsphere(max_articles=3)
    print(f"Found {len(articles)} articles from ZimSphere")
