...
def scrape_newzwire(max_articles: int = 12) -> List[Dict]:
    articles: List[Dict] = []

    start_urls = [
        "https://newzwire.live/",
        "https://www.newzwire.live/",
        "https://newzwire.live/category/news/",
        "https://newzwire.live/category/business/",
        "http://newzwire.live/",
    ]
    ...
    if not soup or not base_url:
        logger.error("NewZWire: no homepage accessible")
        return articles
    ...
    if not link_elems:
        ...
        for a in anchors:
            ...
            if any(x in h for x in ["/category/", "/tag/", "/author/", "/page/"]):
                continue
            if (host in h or "newzwire.live" in h) and ("/20" in h or "/news/" in h):
                link_elems.append(a)
    ...
    for a in link_elems:
        ...
        try:
            ...
            content_html = body_html + "\n" + build_source_attribution(href, "NewZWire")
            ...
            logger.warning(f"NewZWire item failed: {e}")
            continue
...
def scrape_webpage(url: str = None, max_articles: int = 10):
    ...
    articles = scrape_newzwire(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    items = scrape_newzwire(8)
    print(f"Found {len(items)} NewZWire articles")
