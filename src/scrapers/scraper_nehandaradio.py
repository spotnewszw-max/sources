...
#!/usr/bin/env python3
"""
Nehanda Radio scraper (fast + reliable)
- Prioritizes RSS feed for quick discovery
- Parallel article fetching with short timeouts
- Falls back to lightweight homepage parsing
...
BASE = "https://nehandaradio.com"
SOURCE_NAME = "Nehanda Radio"


def _create_session() -> requests.Session:
    """Fast HTTP client with sane headers."""
    s = requests.Session()
    s.headers.update(
        {
            "User-Agent": (
    ...
    for feed in rss_urls:
        try:
            if feedparser:
                ...
                # Simple XML parse via BeautifulSoup
...
def scrape_nehandaradio(max_articles: int = 12) -> List[Dict]:
    session = _create_session()
    max_articles = max(3, max_articles)

    # 1) Try RSS first for speed/reliability
    links = _collect_from_rss(session, max_articles * 2)

    # 2) If RSS empty, fallback to homepage parsing
    if not links:
    ...
    with futures.ThreadPoolExecutor(max_workers=workers) as ex:
        future_map = {ex.submit(_fetch_article, u, session): u for u in links[: max_articles * 3]}
    ...
    if not results:
        logger.warning("%s: Unable to fetch any articles, increasing attempts", SOURCE_NAME)
        for href in links[:max_articles * 2]:
            try:
                item = _fetch_article(href, session)
                if item:
                    url = item.get("source_url")
                    if url not in taken:
                        taken.add(url)
...
def scrape_webpage(url: str = None, max_articles: int = 10) -> List[Dict]:
    ...
    return scrape_nehandaradio(max_articles=max_articles)
...
def scrape_webpage(url: str = None, max_articles: int = 10):
    ...
    articles = scrape_nehandaradio(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    items = scrape_nehandaradio(6)
    print(f"Found {len(items)} Nehanda Radio articles")
