...
#!/usr/bin/env python3
"""
ExpressMail Zimbabwe scraper (fast + reliable)
...
BASE = "https://expressmail.co.zw"
SOURCE_NAME = "ExpressMail Zimbabwe"
...
def scrape_expressmail(max_articles: int = 12) -> List[Dict]:
    ...
    if not results:
        for href in links[:max_articles]:
            results.append(
                {
                    "title": "Latest from ExpressMail",
                    "content": f"<p><a href='{href}' target='_blank'>Read on ExpressMail</a></p>",
                    "source_url": href,
                    "source": SOURCE_NAME,
                }
            )
...
def scrape_webpage(url: str = None, max_articles: int = 10) -> List[Dict]:
    ...
    articles = scrape_expressmail(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    items = scrape_expressmail(6)
    print(f"Found {len(items)} ExpressMail articles")
