...
#!/usr/bin/env python3
"""
NewsHub Zimbabwe scraper with enhanced features and image support
"""
...
def scrape_newshubzim(max_articles=10):
    """
    Scrape NewsHub Zimbabwe with image support
    """
    articles = []
    
    # Multiple URLs to try
    urls_to_try = [
        "https://newshubzim.co.zw/",
        "https://www.newshubzim.co.zw/",
        "https://newshubzim.co.zw/category/news/",
        "https://www.newshubzim.co.zw/category/news/"
    ]
    
    # Use RobustSession (prefers cloudscraper if available)
    from utils.scraper_enhanced import RobustSession
    session_helper = RobustSession("NewsHub Zimbabwe")
    ...
    for url in urls_to_try:
        try:
            logger.info(f"Trying NewsHub Zimbabwe URL: {url}")
            ...
            if response.status_code == 200:
                ...
                successful_url = url
                logger.info(f"Successfully accessed NewsHub Zimbabwe: {url}")
                break
            else:
    ...
    if not soup:
        logger.error("Could not access any NewsHub Zimbabwe URLs")
        return articles
    ...
    try:
        ...
        if not article_containers:
            ...
            for link in all_links:
                ...
                if (text and len(text) > 20 and len(text) < 200 and
                    ('newshubzim.co.zw' in href or href.startswith('/')) and
        ...
        for i, container in enumerate(article_containers):
            ...
            try:
                ...
                logger.info(f"Processing NewsHub Zimbabwe article: {title[:50]}...")
                
                # Get article content and image
                content, image_url = get_newshubzim_article_content(link, scraper)
                ...
                logger.warning(f"Error processing NewsHub Zimbabwe article {i}: {e}")
                continue
        
        logger.info(f"Successfully scraped {len(articles)} articles from NewsHub Zimbabwe")
        return articles
        
    except Exception as e:
        logger.error(f"Error scraping NewsHub Zimbabwe: {e}")
        return articles

def get_newshubzim_article_content(url, scraper):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if content_parts:
                ...
                source_attribution = f"<p><em>Source: <a href='{url}' target='_blank'>NewsHub Zimbabwe</a></em></p>"
...
def scrape_webpage(url: str = None, max_articles: int = 10) -> List[Dict]:
    ...
    articles = scrape_newshubzim(max_articles=max_articles)
...
if __name__ == "__main__":
    ...
    articles = scrape_newshubzim(max_articles=3)
    print(f"Found {len(articles)} articles from NewsHub Zimbabwe")
