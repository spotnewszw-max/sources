...
def scrape_healthtimes(max_articles=10):
    articles = []
    url = "http://healthtimes.co.zw"
...
def scrape_webpage(url: str = None, max_articles: int = 10) -> List[Dict]:
    ...
    return scrape_healthtimes(max_articles=max_articles)

def scrape_webpage(url: str = None, max_articles: int = 10):
    """Wrapper function for compatibility with testing framework"""
    from typing import List, Dict
    articles = scrape_healthtimes(max_articles=max_articles)
