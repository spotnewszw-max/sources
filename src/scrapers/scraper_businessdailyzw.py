...
def scrape_businessdailyzw(max_articles=10):
    """Scrape Businessdailyzw"""
    articles = []
    
    try:
        url = "https://www.businessdaily.co.zw/"
        
        # Use smart_get with SSL bypass for this problematic site
        response = smart_get(url, max_retries=3, timeout=15, try_ssl_bypass=True)
        
        if response is None:
            logger.error("Could not access businessdaily.co.zw")
            return articles
        
        if response.status_code != 200:
            logger.error(f"businessdaily.co.zw returned status {response.status_code}")
            return articles
        ...
        logger.error(f"Error scraping Businessdailyzw: {e}")
        return articles

if __name__ == "__main__":
    articles = scrape_businessdailyzw(max_articles=5)
